...
struct expty transVar(S_table venv, S_table tenv, A_var v, Tr_level l, Temp_label label)
{
	...
	case A_simpleVar:
	{
		...
		{
			struct expty result = expTy(Tr_simpleVar(x->u.var.access, l), actual_ty(x->u.var.ty));
			return result;
		}
		else
		{
	...
	case A_fieldVar:
	{
		...
		while (record)
		{
			...
			{
				return expTy(Tr_fieldVar(varty.exp, index), actual_ty(record->head->ty));
			}
	...
	case A_subscriptVar:
	{
		...
		return expTy(Tr_subscriptVar(varty.exp, expty.exp), actual_ty(varty.ty->u.array));
	}
	}
	assert(0);
}
...
struct expty transExp(S_table venv, S_table tenv, A_exp a, Tr_level l, Temp_label label)
{
	...
	case A_recordExp:
	{
		...
		while (fields && records)
		{
			...
			{
				if (!((actual_ty(field->ty)->kind == Ty_record) &&
							(actual_ty(actual.ty)->kind == Ty_nil)))
				{
	...
	case A_assignExp:
	{
		...
		{
			if (!((actual_ty(assign_var_ty.ty)->kind == Ty_record) &&
						(actual_ty(assign_exp_ty.ty)->kind == Ty_nil)))
			{
	...
	case A_ifExp:
	{
		...
		{
			if (actual_ty(then_ty.ty)->kind != Ty_void && actual_ty(then_ty.ty)->kind != Ty_record)
			{
				EM_error(then->pos, "if-then exp's body must produce no value %d", actual_ty(then_ty.ty)->kind);
			}
		}

		return expTy(Tr_ifthenelse(test_ty.exp, then_ty.exp, elsee_ty.exp), actual_ty(then_ty.ty));
	}
	case A_whileExp:
	{
